...
const LoginPage = ({ onLogin }: { onLogin: (user: UserProfile) => void }) => {
  ...
  const handleGoogleLogin = async () => {
    if (!isSupabaseConfigured()) {
      setError("Supabase n'est pas configuré pour l'authentification Google.");
      return;
    }
    try {
      ...
      >
        ...
        <div className="mt-6">
          ...
          <button
            ...
            type="button"
            className="mt-6 w-full flex items-center justify-center gap-3 py-3 border border-slate-200 rounded-xl font-bold text-slate-700 hover:bg-slate-50 transition-all"
          >
    ...
    <div className="min-h-screen bg-[var(--color-alabaster)] flex items-center justify-center p-6 pt-24 relative overflow-hidden">
      ...
      >
        ...
        <div className="mt-8 pt-8 border-t border-slate-100 text-center">
          <p className="text-slate-500 text-sm">
            Déjà un compte ?{' '}
            <Link to="/login" className="text-[var(--color-majorelle)] font-bold hover:underline">
              {t('login')}
            </Link>
          </p>
        </div>
